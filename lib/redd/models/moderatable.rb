...
module Redd
  module Models
    ...
    module Moderatable
      ...
      def approve() = @client.post('/api/approve', id: @attributes.fetch(:name))

      # Remove a submission.
      # @param spam [Boolean] whether or not this item is removed due to it being spam
      def remove(spam: false) = @client.post('/api/remove', id: @attributes.fetch(:name), spam:)
      ...
      def distinguish(how = :yes)
        params = { id: @attributes.fetch(:name), how: }
      ...
      def ignore_reports() = @client.post('/api/ignore_reports', id: @attributes.fetch(:name))

      # Start getting moderator-related reports on the thing again.
      def unignore_reports() = @client.post('/api/unignore_reports', id: @attributes.fetch(:name))
    end
  end
end
